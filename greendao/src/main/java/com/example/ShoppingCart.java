...
public class ShoppingCart {
    public static void main(String[] args) {
        ...
        Entity customer = schema.addEntity("ShoppingCart");
        //添加ID，自动增长，主键
        Property id = customer.addIdProperty().getProperty();
        //添加属性customerName，字符串类型，不能为空
        customer.addStringProperty("serviceTitle").notNull();
        customer.addStringProperty("name").notNull();
        customer.addStringProperty("originalPrice").notNull();
        customer.addStringProperty("imageUrl").notNull();
        customer.addIntProperty("buyNum").notNull();
        customer.addIntProperty("minBuyNum").notNull();
